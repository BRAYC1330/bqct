...
async def prepare(ctx: RunContext, client, llm, task) -> List[Dict[str, Any]]:
    ...
    for attempt in range(3):
        if kw:
            ...
            if search_data:
                logger.info(f"[search] Fetched {len(search_data.split(chr(10)))} results")
                sample = "\n".join(search_data.split("\n")[:3])
                if generator.validate_search_results(llm, clean_query, sample):
                    break
                logger.info(f"[search] Validation failed for '{kw}', retrying...")
                search_data = ""
        else:
            logger.info(f"[search] No keyword, regenerating (attempt {attempt+1})")
    ...
    if intent == "CASUAL":
        ...
        reply = generator.get_answer(llm, f"[ROOT]\n{clean_root}\n{clean_search}", clean_query, max_chars=max_reply_chars, temperature=config.LLM_TEMP_STANDARD, prompt_key="community_reply")
