import httpx, logging, config, utils
...
async def fetch_trends() -> List[Dict]:
    async with httpx.AsyncClient(timeout=config.SEARCH_TIMEOUT) as client:
        ...
        data = r.json()
        return sorted([t for t in data.get("items", []) if utils.is_english(t.get("summary", ""))], key=lambda x: x.get("current_rank", 999))[:10]

@retry_async()
async def fetch_chainbase(keyword: str) -> str:
    async with httpx.AsyncClient(timeout=config.SEARCH_TIMEOUT) as client:
        r = await client.get("https://api.chainbase.com/tops/v1/tool/search-narrative-candidates", params={"keyword": keyword})
        if r.status_code != 200: return ""
        ...
        for i in items:
            ...
            seen.add(key); valid.append((kw, sm))
...
async def fetch_mentions(keyword: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=config.SEARCH_TIMEOUT) as client:
        for kw in kw_list:
            try:
                r = await client.get("https://api.chainbase.com/tops/v1/tool/search-mentions", params={"keyword": kw})
