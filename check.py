...
async def run():
    ...
    try:
        ...
        logger.info(f"[checker] Fetched {len(notifs)} notifications")
        for n in notifs:
            idx = n.get("indexedAt", "")
    ...
    if manual in ("mini", "full"):
        ...
        try:
            ...
            diff_hours = (now_utc - last_dt).total_seconds() / 3600
    ...
    write_outputs(status='true' if has_tasks else 'false', tasks=tasks_json, state_json=state.model_dump_json(), scheduled_type=scheduled_type or "")
