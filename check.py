import os, sys, json, asyncio, logging, httpx
...
async def run():
    ...
    try: state_data = json.loads(last_processed_raw) if last_processed_raw else {}
    except json.JSONDecodeError: state_data = {}
    ...
    if not state.seen_at: state.seen_at = now_utc_str
    ...
    try:
        ...
        for n in notifs:
            ...
            seen_uris.add(uri)
            if n.get("reason") not in ("reply", "mention"): continue
            ...
            if author_did == config.OWNER_DID and n.get("reason") == "reply":
                if uri == root_uri and f"@{config.BOT_HANDLE.replace('@', '')}" not in text: continue
                if state.digest_uri and root_uri == state.digest_uri and parent_uri != state.digest_uri: continue
                if state.digest_uri and "/app.bsky.feed.post/" in root_uri:
                    if root_uri == state.digest_uri:
                        ...
                        digest_comment_count += 1; continue
                    else: continue
            ...
            if state.digest_uri and root_uri == state.digest_uri:
                if parent_uri and parent_uri != state.digest_uri and parent_uri.startswith(f"at://{config.BOT_DID}/"): continue
                tasks.append(Task(type=TaskType.digest_comment, uri=uri, text=text, author_did=author_did, parent_uri=parent_uri, embed=record.get("embed")))
                digest_comment_count += 1; continue
    ...
    if manual in ("mini", "full"):
        ...
        except Exception: scheduled_type = "mini"
    else: scheduled_type = "mini"
